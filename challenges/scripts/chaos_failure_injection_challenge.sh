...
# for the Containers submodule per CONST-035 + CONST-050(B).
# Submodule cascade per CONST-051(A). Client-side fault injection.

set -uo pipefail

HEALTH_URL="${CONTAINERS_HEALTH_URL:-}"
CHAOS_HOST="${CONTAINERS_CHAOS_HOST:-localhost}"
CHAOS_PORT="${CONTAINERS_CHAOS_PORT:-}"
LEGIT_REQS="${CHAOS_LEGIT_REQUESTS:-100}"
MIN_PCT="${CHAOS_LEGIT_MIN_PASS_PCT:-95}"

echo "=== Containers Chaos Failure-Injection Challenge ==="
echo "  url=$HEALTH_URL host=${CHAOS_HOST}:${CHAOS_PORT}"

if [[ -z "$HEALTH_URL" ]] || [[ -z "$CHAOS_PORT" ]]; then
    echo "[1/6] SKIP: CONTAINERS_HEALTH_URL/_PORT unset — SKIP-OK: #env-no-target"
    echo "=== Containers Chaos Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
[[ "$post" != "200" ]] && { echo "[2/6] FAIL: tipped over (HTTP $post)"; exit 1; }
...
echo
echo "=== Containers Chaos Challenge: PASSED ==="
