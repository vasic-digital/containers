...
echo "=== Containers Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS} conc=${CONCURRENCY}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: CONTAINERS_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== Containers Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
pct=$((ok * 100 / total))
echo "[4/6] Sustained: total=$total ok=$ok pct=${pct}% wall=${wall}s"
...
[[ "$post" != "200" ]] && { echo "[6/6] FAIL: post HTTP $post"; exit 1; }
echo "[6/6] Post-stress liveness: PASS"

echo
echo "=== Containers Stress Challenge: PASSED ==="
